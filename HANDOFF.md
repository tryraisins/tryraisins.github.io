...
- `src/prototypes/portfolio-redesign/doodle-notes.js` contains 20 sketch definitions and a draw/hold/reverse lifecycle. Each doodle now has a distinct secondary hold animation (for example: cat tail sway, coffee steam, bicycle spokes, cassette reels, whale spout); reduced-motion continues to show completed static sketches.
...
- Seven transparent display-only canvases span the hero, work area, and contact area. Each draws for 5.2 seconds, holds for exactly 11 seconds, erases over 2.6 seconds, then advances to another sketch. During the hold, each doodle has a contextual secondary movement. Reduced-motion presents the completed drawing without animation.
